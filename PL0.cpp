...
int main()
{
	...
	if (fin)
	{
		...
		// scanf("%s", fname);
		fname[0] = 'Y';//strscp(fname, 'Y'); // 关闭可选输入，默认输出所有文件
...
{
	int i, j, k;
	while (ch == ' ' || ch == 10 || ch == 9) /*忽略空格和tab等 */
	{
		getchdo; // 执行getch(); 宏定义
	}
	if (ch >= 'a' && ch <= 'z') /*名字或保留字以a~z开头 */
	{
		k = 0; // k用于记录每个sym中的当前位置
		do
		{
			if (k < al) // 起名不能超过符号最大长度，否则只读取前面的
			{
				a[k] = ch;
				k++;
			}
		...
		a[k] = 0; // a数组记录了sym，并以0结尾
		strcpy(id, a); // 复制到id上，后续操作用id而不是a
		i = 0; // 此处用于表示word数组的头结点
		j = norw - 1; // 关键字个数-1，表示word数组的尾节点
		do
		{ /*二分查找该关键词id，（就norw=14个字还二分查找，没病吧。。）*/
			k = (i + j) / 2;
			...
			else
			{
				...
				else
				{
					if (sym == ifsym) /*准备按照if语句处理*/
					{
						...
						if (sym == elsesym)			 /*发现es关键字*/
						{
							getsymdo;
							cx2 = cx;
							code[cx1].a = cx + 1; /*cx为当前的指令地址，
                                              cx+1即为then语句执行后的se语句的位置，回填地址*/
							gendo(jmp, 0, 0);
							statementdo(fsys, ptx, lev); /*处理es后的语句*/
							code[cx2].a = cx;			 /*经statement处理后，cx为se后语句执行
